...
def val_epoch(model, dataloader, device, criterion):
    model.eval()
...
def train(model, X_data, y_data, V_data,v_y_data, tokenizer, epochs, lr, bs, device,patience=3):
    ...
    val_dataset = EmotionDataset(V_data, v_y_data, tokenizer)
    val_dataloader = DataLoader(val_dataset, num_workers = 0, batch_size=bs, collate_fn=collate_fn,shuffle=True)
    ...
    for epoch in range(epochs):
        ...
        # ===== VALIDATION =====model, dataloader, device, criterion
        val_loss, val_acc, val_f1 = val_epoch(model, val_dataloader, device, criterion)
        ...
        if val_f1 > best_f1:
            ...
            torch.save(model.state_dict(), "src/transformer/best_transformer.pt")
        ...
        print(f'Epochs: {epoch + 1} | Loss: {avg_train_loss: .3f} | Accuracy: {train_acc: .3f}')
